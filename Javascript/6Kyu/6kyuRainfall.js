...
MEAN:
P: town, strng (data or data1)
R: if no town found, 1. Otherwise return the average of rainfall

let line = strng 
.split ('\n')
.find(l => l.statsWith(town + ":"))

now we have the line, we need to split it into months, and then find the numbers,
...
function mean(town, strng) {
  ...
  let line = strng.split('\n')
                  .find(l => l.startsWith(town + ":"));
...
function variance(town, strng) {
  ...
  let nums = line.split(":")[1]
   ...
   let mean = nums.reduce((acc, x) => acc + x, 0) / nums.length;
